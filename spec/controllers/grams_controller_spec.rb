...
RSpec.describe GramsController, type: :controller do
	describe "grams#destroy action" do
		...
		it "should't let unauthenticated users destroy a gram" do
			gram = FactoryGirl.create(:gram)
	...
	describe "grams#update action" do
		it "shouldn't let users who didn't create the gram update it" do
			...
			patch :update, params: { id: gram.id, gram: { message: "my message" } }
			expect(response).to have_http_status(:forbidden)
		end
		...
		it "should have http 404 error if the gram cannot by found" do
			user = FactoryGirl.create(:user)
			sign_in user
	...
	describe "grams#edit action" do
		...
		it "should return a 404 error message if the gram is not fouind" do
			user = FactoryGirl.create(:user)
			sign_in user
